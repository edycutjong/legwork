...
import { CONTRACT, DEPLOY, OVERHEAD, chainOk } from './app/rpc';
...
app.append(
  h('div', { class: 'sheet' },
    h('header', { class: 'masthead' },
      ...
      h('nav', {}, h('a', { href: '#/' }, 'orders'), h('a', { href: `#/o/${DEPLOY.orders?.live?.id ?? 1}` }, 'try the live order'), h('a', { href: explorerAddress(CONTRACT), target: '_blank', rel: 'noopener' }, `contract ${short(CONTRACT)}`), walletSlot)),
    main,
    h('footer', { class: 'foot' },
      h('span', {}, `Arc mainnet · chain 5042 · contract `, h('a', { href: explorerAddress(CONTRACT), target: '_blank', rel: 'noopener' }, CONTRACT)),
      h('span', {}, `OVERHEAD ${OVERHEAD} gas · REFUND_CEIL_GAS 120,000 · payee stipend 30,000`),
